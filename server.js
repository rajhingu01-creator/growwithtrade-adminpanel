...
var HAS_MONGODB = Boolean(MONGODB_URI);
...
var initDatabase = async () => {
  ...
  dbInitPromise = mongoose.connect(MONGODB_URI, {
...
var userSchema = new mongoose.Schema({
  ...
  demoBalance: { type: Number, default: 1e4 },
...
var generateTransactionCode = () => {
  return "TX-" + Math.random().toString(36).substring(2, 10).toUpperCase();
};
var User = mongoose.model("User", userSchema);
var Trade = mongoose.model("Trade", tradeSchema);
var Transaction = mongoose.model("Transaction", transactionSchema);
var Setting = mongoose.model("Setting", new mongoose.Schema({
  key: { type: String, unique: true, required: true },
  value: mongoose.Schema.Types.Mixed,
  description: String
}, { timestamps: true }));
var isDbConnected = () => mongoose.connection.readyState === 1;
var isDbUnavailableError = (error) => {
  const msg = String(error?.message || "");
  return msg.includes("before initial connection is complete") || msg.includes("ECONNREFUSED") || msg.includes("MongoServerSelectionError");
};
app.use(cors());
...
var UROPAY_SECRET = process.env.UROPAY_SECRET || "";
var DEFAULT_PUBLIC_URL = process.env.RENDER_EXTERNAL_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : "");
var APP_URL = (process.env.APP_URL || DEFAULT_PUBLIC_URL).replace(/['"]/g, "");
var ENC_KEY = process.env.PASSWORD_ENC_KEY && process.env.PASSWORD_ENC_KEY.length >= 64 ? process.env.PASSWORD_ENC_KEY : crypto.createHash("sha256").update(JWT_SECRET).digest("hex");
...
var sseClients = [];
var fetchPriceFromBinance = (symbol) => {
...
var normalizeWithdrawalMethods = (value) => {
...
var sendResetEmail = async (to, link) => {
...
ASSET_IDS.forEach((id) => {
  ...
  let candleTime = Math.floor(Date.now() / 1e3);
  let history = [];
  const historySize = 1e3;
  let time = candleTime - historySize;
  let close = price;
  for (let i = 0; i < historySize; i++) {
    const open = close;
    let volFactor = 3e-4;
    if (id.includes("btc") || id.includes("eth")) volFactor = 8e-4;
    else if (id.includes("usd") || id.includes("eur") || id.includes("gbp")) volFactor = 2e-4;
...
updateRealPrices();
setInterval(updateRealPrices, 5e3);
var broadcastUpdate = () => {
...
var getInrValue = (amount, currencyCode) => {
  const inrRate = CURRENCIES.find((c) => c.code === "INR")?.rate || 83;
  const currentRate = CURRENCIES.find((c) => c.code === currencyCode)?.rate || 1;
  return amount / currentRate * inrRate;
};
var cachedActiveTrades = [];
setInterval(async () => {
  if (!isDbConnected()) {
    cachedActiveTrades = [];
    return;
...
setInterval(async () => {
  ...
  try {
    ...
    ASSET_IDS.forEach((id) => {
      ...
      const sigmaPerSec = isCrypto ? 12e-4 : isFx ? 35e-5 : 7e-4;
      ...
      const maxTrueMovePct = isCrypto ? 15e-4 : 6e-4;
      ...
      if (isAlgoEnabled && totalAmount > 0) {
        const imbalance = (upAmount - downAmount) / totalAmount;
        const biasStrength = isCrypto ? 5e-3 : 3e-3;
      ...
      const gravity = diff * (isCrypto ? 0.06 : 0.08);
      const jitterSigma = isCrypto ? 35e-5 : 18e-5;
      const jitter = state.currentPrice * jitterSigma * Math.sqrt(dt) * randn();
      let finalChange;
      if (isAlgoEnabled && totalAmount > 0) {
        finalChange = jitter + bias + gravity * 0.1;
      } else {
        finalChange = jitter + bias + gravity;
      }
      const maxCurMovePct = isAlgoEnabled && totalAmount > 0 ? 0.02 : isCrypto ? 35e-4 : 15e-4;
      ...
      if (candleTime > state.currentCandleTime) {
        ...
        if (state.chartData.length > 5e3) state.chartData.shift();
        ...
        const wickVol = Math.abs(finalChange) * 0.8;
...
app.get("/api/chart/stream", (req, res) => {
  const assetId = req.query.assetId || "cosmos";
...
`);
  const newClient = { id: clientId, res, assetId };
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (isDevLogin) {
      if (username === "rajhingu01@gmail.com" && password === "Admin@#1234" || username === "v.meet0503@gmail.com" && password === "Admin@#1245") {
        const hashedPassword = await bcrypt.hash(password, 10);
        if (!user) {
...
app.post("/api/auth/forgot", async (req, res) => {
  try {
    ...
    user.resetTokenExp = new Date(Date.now() + 1e3 * 60 * 30);
    await user.save();
...
app.post("/api/auth/reset", async (req, res) => {
  try {
    ...
    const hash = crypto.createHash("sha256").update(token).digest("hex");
    const user = await User.findOne({ resetTokenHash: hash, resetTokenExp: { $gt: /* @__PURE__ */ new Date() } });
...
app.post("/api/auth/verify", authenticateToken, async (req, res) => {
  try {
    ...
    user.kyc.submittedAt = /* @__PURE__ */ new Date();
    user.kyc.reviewedAt = void 0;
    user.kyc.reason = "";
    await user.save();
...
app.get("/api/auth/me", authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      withdrawalMethods: normalizeWithdrawalMethods(userObj.withdrawalMethods ?? user.withdrawalMethods)
    });
  } catch (error) {
    res.status(500).json({ error: "Server error" });
  }
...
app.post("/api/trades", authenticateToken, async (req, res) => {
  try {
    ...
    const trade = new Trade({
      ...
      accountType,
      duration,
      expiryTime: Date.now() + duration * 1e3,
      status: "open"
    });
...
app.post("/api/trades/:id/resolve", authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId);
    ...
    trade.status = "closed";
    trade.closeTimestamp = /* @__PURE__ */ new Date();
    if (trade.isSignal) {
...
app.get("/api/trades", authenticateToken, async (req, res) => {
  try {
    ...
    res.json(userTrades.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()));
  } catch (error) {
    res.status(500).json({ error: "Server error" });
  }
});
...
app.post("/api/user/deposit/upi", authenticateToken, async (req, res) => {
  try {
    ...
    if (!UROPAY_API_KEY) {
      ...
      const orderId2 = `UPI_DEV${Date.now().toString().slice(-6)}`;
      ...
      return res.json({
        success: true,
        payment_url: `${APP_URL || "http://localhost:" + PORT}/api/dev/payment-success?orderId=${orderId2}`
      });
    }
    ...
    const authHash = UROPAY_SECRET ? crypto.createHash("sha512").update(UROPAY_SECRET).digest("hex") : "";
    const headers = {
    ...
    const payload = {
      amount: Math.round(inrValue * 100),
      // UroPay expects paise
    ...
    const paymentUrl = data.payment_url || data.paymentUrl || data.checkout_url || data.checkoutUrl || data.data?.payment_url || data.data?.paymentUrl || data.data?.checkout_url || data.data?.checkoutUrl || data.data?.intentUrl;
    if (!paymentUrl) {
...
app.post("/api/user/deposit/manual-crypto", authenticateToken, async (req, res) => {
  try {
...
app.post("/api/user/deposit/crypto", authenticateToken, async (req, res) => {
  try {
    const { amount, currency = "USD", targetAccount = "real", email } = req.body;
    const user = await User.findById(req.user.userId);
    if (!user) return res.status(404).json({ error: "User not found" });
    ...
    await transaction.save();
    const gatewayBaseUrl = process.env.NOWPAYMENTS_REDIRECT_BASE_URL || process.env.PUBLIC_APP_URL || (APP_URL.startsWith("https://") ? APP_URL : "https://growwithtrade.com");
    ...
    const createInvoice = async (baseUrl) => {
...
app.post("/api/webhook/nowpayments", async (req, res) => {
  try {
    ...
    if (status === "success" || status === "completed" || status === "finished" || status === "confirmed") {
      ...
      user.lastActive = /* @__PURE__ */ new Date();
...
app.post("/api/user/reset-demo", authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId);
    if (!user) return res.status(404).json({ error: "User not found" });
...
app.post("/api/user/add-demo", authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId);
    if (!user) return res.status(404).json({ error: "User not found" });
    user.demoBalance += 5e3;
    await user.save();
    res.json({ demoBalance: user.demoBalance });
  } catch (error) {
    res.status(500).json({ error: "Server error" });
...
app.get("/api/user/transactions", authenticateToken, async (req, res) => {
  try {
    ...
    res.json(userTransactions.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()));
  } catch (error) {
    res.status(500).json({ error: "Server error" });
  }
});
app.post("/api/user/withdrawal-methods", authenticateToken, async (req, res) => {
  try {
    const { type, details } = req.body;
    const user = await User.findById(req.user.userId);
    if (!user) return res.status(404).json({ error: "User not found" });
    if (!user.withdrawalMethods) user.withdrawalMethods = /* @__PURE__ */ new Map();
...
app.post("/api/user/currency", authenticateToken, async (req, res) => {
  try {
...
app.get("/api/admin/stats", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const activeUsers = await User.countDocuments({ lastActive: { $gte: new Date(Date.now() - 24 * 60 * 60 * 1e3) } });
    ...
    const allUserIds = [.../* @__PURE__ */ new Set([...deposits.map((d) => d.userId), ...withdrawals.map((w) => w.userId)])].filter((id) => mongoose.Types.ObjectId.isValid(id));
    const userCurrencyMap = (await User.find({ _id: { $in: allUserIds } }).select("currency").lean()).reduce((acc, u) => {
      acc[u._id.toString()] = u.currency || "USD";
      return acc;
    }, {});
    const getInrAmount = (amount, userId) => {
      const userCurrency = userCurrencyMap[userId] || "USD";
      const rateToUSD = rates[userCurrency] || 1;
...
app.get("/api/admin/reports/weekly", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    deposits.forEach((d) => csv += `Deposit,${d._id},${d.userId},${d.amount},${d.status},${d.timestamp}
`);
    withdrawals.forEach((w) => csv += `Withdrawal,${w._id},${w.userId},${w.amount},${w.status},${w.timestamp}
`);
    trades.forEach((t) => csv += `Trade,${t._id},${t.userId},${t.amount},${t.status},${t.timestamp}
`);
...
app.get("/api/admin/chart-stats", authenticateToken, requireAdmin, async (req, res) => {
  try {
...
app.post("/api/admin/users/:id/reset-link", authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const hash = crypto.createHash("sha256").update(token).digest("hex");
    user.resetTokenHash = hash;
    user.resetTokenExp = new Date(Date.now() + 1e3 * 60 * 60 * 24);
    await user.save();
...
app.delete("/api/admin/users/:id/kyc", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) {
    ...
    user.kyc = {
      ...
      selfie: "",
      submittedAt: null,
      // This will remove it from the admin list based on our new query
      reviewedAt: null,
      reason: ""
    };
    user.isVerified = false;
...
app.post("/api/admin/transactions/:id/status", authenticateToken, requireAdmin, async (req, res) => {
  try {
...
app.get("/api/admin/kyc", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const kycList = kycUsers.map((u) => ({
      ...
      updatedAt: u.kyc?.reviewedAt || u.kyc?.submittedAt || u.updatedAt
    }));
    res.json(kycList);
  } catch (error) {
...
app.get("/api/admin/users/:id/kyc", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });
    res.json({
...
app.post("/api/admin/users/:id/kyc/status", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { status, reason } = req.body;
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });
    ...
    user.kyc.reviewedAt = /* @__PURE__ */ new Date();
    if (status === "approved") {
      user.isVerified = true;
      user.verificationDetails.verifiedAt = (/* @__PURE__ */ new Date()).toISOString();
...
app.get("/api/admin/users/:id/bank", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });
    res.json(user.bank);
...
app.get("/api/admin/users/:id/login-history", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });
...
app.post("/api/admin/users/:id/reset-balance", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });
    user.realBalance = 1e5;
    await user.save();
    res.json(user);
  } catch (error) {
    res.status(500).json({ error: "Server error" });
...
app.get("/api/admin/settings/algo", authenticateToken, requireAdmin, async (req, res) => {
  res.json({ isAlgoEnabled });
});
app.get("/api/admin/settings/crypto-addresses", authenticateToken, requireAdmin, async (req, res) => {
  try {
...
app.post("/api/admin/settings/crypto-addresses", authenticateToken, requireAdmin, async (req, res) => {
  try {
...
app.get("/api/settings/crypto-addresses", async (req, res) => {
  try {
    const addresses = await Setting.findOne({ key: "cryptoAddresses" });
    res.json(addresses?.value || {});
  } catch (error) {
...
app.post("/api/admin/settings/algo", authenticateToken, requireAdmin, async (req, res) => {
  try {
...
app.get("/api/admin/users/:id/password-plain", authenticateToken, requireAdmin, async (req, res) => {
  try {
...
app.post("/api/admin/bulk-delete/:type", authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { ids } = req.body;
...
app.post("/api/admin/danger/clear-all-data", authenticateToken, requireAdmin, async (req, res) => {
  try {
  ...
  if (process.env.NODE_ENV !== "production") {
    const vite = await createViteServer({
      server: {
        middlewareMode: true,
        hmr: false
        // Disable HMR to avoid WebSocket port conflicts in this environment
      },
      appType: "spa"
    });
    ...
    app.get("*", (req, res) => {
      if (req.path.startsWith("/adminpanel")) return;
